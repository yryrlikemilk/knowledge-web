...
const KnowledgeFile = () => {
  ...
  // 点击处理函数（暂时仅输出 console，后续补充具体逻辑）
  const handleReparse = async () => {
    if (!runStatus?.doc_ids || runStatus.doc_ids.length === 0) {
      message.warning('没有可重新解析的文档');
      return;
    }
    try {
      const body = {
        delete: true,
        document_ids: runStatus.doc_ids,
        run: 1,
      };
      const resp = await request.post('/api/file/run', { data: body });
      if (resp && (resp.code === 0 || resp.status === 0 || resp.success === true)) {
        message.success('操作成功');
      } else {
        message.error(resp?.message || '重新解析请求失败');
      }
    } catch (err) {
      console.error('重新解析出错', err);
      message.error('重新解析出错，请重试');
    }
  };
